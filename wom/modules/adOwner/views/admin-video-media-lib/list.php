...
AppAsset::addCss($this, '@web/src/css/ad-owner-user-admin/video-media-lib-manage.css');
...
AppAsset::addScript($this, '@web/src/js/ad-owner-user-admin/video-media-lib-manage.js');
$this->title = '视频媒体库管理';
?>
<!--面包屑-->
...
<div class="content fr">
    ...
    <input id="id-delete-excel-url" type="hidden" value="<?= Url::to(['/ad-owner/admin-video-media-lib/delete-export-excel']) ?>">
...
JS;
    ...
    <div class="con-top shadow clearfix">
        ...
        <div class="condition-area detail-name fl font-16 font-500">艺人名称 :
            <input type="text" name="platform_name" value="<?php echo Yii::$app->request->post('platform_name', ''); ?>" placeholder="输入艺人名称">
        </div>
            ...
            if(!empty($dataProvider)){
                foreach($dataProvider as $key => $group){?>
                    <tr data-uuid="<?=$group['group_uuid']?>">
                        <td>
                            <a  href="<?=Url::to(['/ad-owner/admin-video-media-lib/detail','group_uuid'=>$group['group_uuid']])?>" class="plain-text-length-limit" data-limit="7" data-title="<?=$group['group_name']?>">
                                <?=$group['group_name']?>
                            </a>
                        </td>
                        <td><?=$group['media_cnt']?></td>
                        <td><?=$group['total_fan_cnt']?></td>
                        <td><?=json_decode($group['extra_data'])->total_avg_watch_num;?></td>
                        <td><?=date('Y-m-d',$group['last_update_time'])?></td>
                        <td>
                            <a class="manage" href="<?=Url::to(['/ad-owner/admin-video-media-lib/detail','group_uuid'=>$group['group_uuid']])?>">管理</a>
                            <a class="export" href="javascript:;" data-url="<?=Yii::$app->urlManager->createUrl(array('/ad-owner/admin-video-media-lib/export-lib-media'))?>" data-uuid="<?=$group['group_uuid']?>">导出</a>
                            <a class="remove" href="javascript:;" data-url="<?=Yii::$app->urlManager->createUrl(array('/ad-owner/admin-video-media-lib/delete-group'))?>" data-uuid="<?=$group['group_uuid']?>">移除</a>
                        </td>
                    </tr>
                <?php }} ?>
            </tbody>
