...
function pjaxRefVideoLibDetail(){
    //鼠标放上去显示完整信息
    ...
    $('.little-code').hover(function(){
        $(this).parents().siblings(".video-code").css({display:'block'})
    },function(){
        $(this).parents().siblings(".video-code").css({display:'none'})
    })

    //~~~~~~判断有无资源~~~~~~
    function isResource(){
        var resourceLength =  $(".video-resource-list-table tbody").children("tr").length;
        if(resourceLength < 1){
    ...
    $(".btn-new-lib-save").click(function(){
        ...
        var video_lib_url = $('#id-video-lib-url').val();
            ...
            $.ajax({
                ...
                success: function (resp) {
                    if (resp.err_code == 0) {
                        ...
                        $("#create").modal("hide");
                        window.location.href = video_lib_url;
                    } else {
                        wom_alert.msg({
                            icon: "error",
                            content: "系统错误，请联系管理员!",
    ...
    //删除媒体库资源
    $(".video-resource-list-table").on("click",".delete",function(){
    ...
    // 重新分组 - 弹框打开
    $(".video-resource-list-table .re-group").on("click", function(){
        // 获取分组列表
        var get_all_media_lib_url = $('#id-get-all-video-media-lib-url').val();
        var media_uuid = $(this).data("item-uuid");
            ...
            success: function (resp) {
                if (resp.err_code == 0) {
                    ...
                    for (var i = 0; i < video_media_lib_list.length; i++) {
                        $("#regrouping .group-name ul ").append('<li data-uuid="' + video_media_lib_list[i].uuid + '"><input class="fl" name="select-group-name" type="checkbox"><span class="fl">' + video_media_lib_list[i].group_name + '</span></li>');
                    }
    ...
    $(".batch-manage").on("click",".add",function(){
        ...
        var video_list_url =  $("#id-video-list-url").val();
        window.location.href = video_list_url+"&group_uuid="+group_uuid;
    });
    ...
    $(".batch-manage").on("click",".export",function(){
        ...
        $(".video-resource-list-table [name='media-selected']").each(function(){
            if($(this).is(':checked')){
                media_selected_cnt++;
        ...
        var item_uuid_arr = Cookies.get('video-media-selected-to-export');//导出资源的cookie
        wom_alert.confirm({
            content: '确定导出媒体库资源吗?'
        },function(){
            $.ajax({
                ...
                success: function (resp) {
                    if (resp.err_code == 0) {
                        ...
                        Cookies.remove('video-media-selected-to-export');
    ...
    // 根据video media uuid从cookie里移除给定的资源
    function deleteMediaInCookie(media_uuid) {
        var video_media_selected_to_export = Cookies.get('video-media-selected-to-export');
        console.log(video_media_selected_to_export);
        if (video_media_selected_to_export != '' && video_media_selected_to_export != undefined  ) {
            video_media_selected_to_export = video_media_selected_to_export.replace(media_uuid + ',', '');
            Cookies.set('video-media-selected-to-export', video_media_selected_to_export, {expires: 7});
        }
    }
    ...
    function addMediaIntoCookie(media_uuid) {
        var video_media_selected_to_export = Cookies.get('video-media-selected-to-export');
        if (video_media_selected_to_export == undefined || video_media_selected_to_export == '') {
            Cookies.set('video-media-selected-to-export', media_uuid + ',');
        } else if (video_media_selected_to_export.indexOf(media_uuid) < 0) {
            Cookies.set('video-media-selected-to-export', video_media_selected_to_export + media_uuid + ',');
        }
    }
