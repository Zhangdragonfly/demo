...
$weixinToVerifyJs = <<<JS
            ...
            // 控制左侧导航选中
            if(!$('#weixin .media-manage .to-verify').hasClass('active')){
                ...
                $('#weixin.menu-level-1 .menu-level-2.media-manage .menu-level-3.to-verify').addClass('active');
            }
                            ...
                            if(resp.err_code == 0){
                                ...
                                for(var i = 0; i < vendorList.length; i++){
                                    ...
                                    if(is_pref_vendor == 0) {
                                        ...
                                        var oneLine = '<tr class="table-line" data-is-prefer="' + is_pref_vendor + '" data-uuid="' + bind_uuid + '" id="' + bind_uuid + '"><td>' + (i + 1) + '</td><td>' + vendor_name + '</td><td>' + register_type_label + '</td><td>' + contact_info_label + '</td><td>' + activate_label + '</td><td class="is-pref-vendor-label">' + is_pref_vendor_label + '</td><td>' + bind_status_label + '</td><td>' + action_cell + '</td></tr>';
                                    } else {
                                        is_pref_vendor_label = '是';
                                        var oneLine = '<tr class="table-line" data-is-prefer="' + is_pref_vendor + '" data-uuid="' + bind_uuid + '" id="' + bind_uuid + '" style="color: #ef0a0a"><td>' + (i + 1) + '</td><td>' + vendor_name + '</td><td>' + register_type_label + '</td><td>' + contact_info_label + '</td><td>' + activate_label + '</td><td class="is-pref-vendor-label">' + is_pref_vendor_label + '</td><td>' + bind_status_label + '</td><td>' + action_cell + '</td></tr>';
                                    }
            ...
            $('#modal-to-verify #stage-to-verify-vendor-content .area-vendor-detail .one-pos.pos-s .pub-type input:radio[name="pos-s-pub-type"]').change(function() {
                ...
                if(pub_type == 0){
                    ...
                    if(!isNaN(origin_price_min) && origin_price_min > 0){
            ...
            //平台合作价同步到执行价
            //var vendorDetailArea = $('#modal-to-verify #stage-to-verify-vendor-content .area-vendor-detail');
            //vendorDetailArea.find('.one-pos.pos-s .orig-price .origin-price-val-min').blur(function(){
            //$('.one-pos.pos-s .orig-price .origin-price-val-min').blur(function(){
            //    vendorDetailArea.find('.one-pos.pos-s .orig-price .execute-price-val').val($(this).val());
            //});
            ...
            $('#modal-to-verify').on('click', '.btn-commit-vendor', function(){
                ...
                swal({
                    ...
                    $.ajax({
                            ...
                            cache: false,
                            dataType: 'json',
                            data: {'bind_uuid': bind_uuid, 'pub_config': pub_config, 'active_end_time': active_end_time, 'deposit_percent_config': deposit_percent, 'serve_percent_config': serve_percent, 'vendor_bind_status': vendor_bind_status, 'pay_period': pay_period, 'coop_level': coop_level, 'is_pref_vendor': is_pref_vendor, 'media_ownership': media_ownership},
                            success: function (resp) {
                                if(resp.err_code == 1){
                                    swal('', '保存失败！', 'error');
                                }else{
...
<div id="content" class="content">
    ...
    <div class="row">
        <div class="col-md-12 main-stage">
            ...
            // ================= 搜索 =================
            $('.pjax-area .weixin-search-form').on('click', '.btn-submit', function() {
...
JS;
            ...
            <div class="panel panel-inverse pjax-area">
                <?= Html::beginForm(['media/to-verify-list'], 'post', ['data-pjax' => '', 'class' => 'weixin-search-form']); ?>
                <div class="p-t-30">
                            ...
                            'columns' => [
                                ...
                                [
                                    ...
                                    'value' => function ($model, $key, $index, $column) {
                                        ...
                                        return "<span>{$model['public_name']}</span><br/><span>{$model['public_id']}</span><br/><br/>粉丝数: {$followNum}万<br/>激活: {$activatedLabel}<br>资源分类: <br>{$mediaCateLabel}";
                                    },
                                ],
//                                [
//                                    'header' => '媒介',
...
//                                ],
                                [
                                    ...
                                    'format' => 'html',
                                    'headerOptions' => ['data-sort-ignore' => 'true'],
                                    'contentOptions' => ['class' => 'is-pref-vendor','style'=>'cursor:pointer;color:#337ab7;'],
                                    'value' => function ($model, $key, $index, $column) {
                                        return ($model['vendor_name'] == '' )? "<a title=联系人：".$model['contact1']."接单备注：".$model['vendor_comment'].">".$model['vendor_contact_person']."</a>": "<a title=联系人：".$model['contact1']."接单备注：".$model['vendor_comment'].">".$model['vendor_name']."</a>";
                                    },
                                ],
                                [
                                    'header' => '价格(元)',
                                    ...
                                    'value' => function ($model, $key, $index, $column) {
                                        $m_1_retailPriceActiveEndTimeLabel = '【价格有效期】<br>';
                                        $m_1_retailPriceActiveEndTimeLabel .= (!empty($model['active_end_time']) && $model['active_end_time'] !=-1 )?date('Y-m-d',$model['active_end_time']).'<br>':"未设置". '<br>';

                                        $origPriceLabel = '【平台合作价】<br>';
                                        ...
                                        return $m_1_retailPriceActiveEndTimeLabel . $origPriceLabel;
                                    },
                                ],
                                [
                                    'header' => '备注',
                                    'format' => 'html',
                                    'headerOptions' => ['data-sort-ignore' => 'true'],
                                    'contentOptions' => ['class' => 'comment', 'style' => 'max-width: 150px;'],
                                    'value' => function ($model, $key, $index, $column) {
                                        $comment = '';
                                ...
                                [
                                    ...
                                    'value' => function ($model, $key, $index, $column) {
                                        if ($model['weixin_status'] == MediaWeixin::STATUS_INFO_TO_VERIFY) {
                                            return '<span class="label label-warning">资源待审核</span>';
                                        }
                                        if ($model['to_verify_vendor_cnt'] > 0) {
                                            return '<span class="label label-warning">媒体主待审核</span>';
                                        }
                                        if ($model['has_pref_vendor'] == 0) {
                                            return '<span class="label label-warning">待设置首选媒体主</span>';
                                        }
                                    }
                                ],
                                [
                                    ...
                                    'value' => function ($model, $key, $index, $column) {
                                        return '入驻时间: <br>' . date('Y-m-d H:i', $model['create_time']) . '<br>' . '审核时间: <br>' . date('Y-m-d H:i', $model['last_verify_time']) . '<br>' . '最后更新: <br>' . date('Y-m-d H:i', $model['last_update_time']);
                                    },
                                ],
                                [
                                    'class' => 'yii\grid\ActionColumn',
                                    'header' => '操作',
                                    'template' => '{to-verify}<br>{to-delete}',
                                    'buttons' => [
                                        ...
                                        'to-verify' => function ($url, $model) {
                                            return Html::button('审核', ['class' => 'btn btn-link btn-xs btn-to-verify', 'data-uuid' => $model['media_uuid']]);
                                        },
                                        'verify-history' => function ($url, $model) {
                                            return Html::button('查看历史', ['class' => 'btn btn-link btn-xs btn-verify-history', 'data-uuid' => $model['media_uuid']]);
                                        },
                                        'to-delete' => function ($url, $model) {
                                            return Html::button('删除', ['class' => 'btn btn-link btn-xs btn-to-delete', 'data-uuid' => $model['media_uuid']]);
                                        }
                                    ],
                                    'headerOptions' => ['data-sort-ignore' => 'true'],
                ...
                <table id="header-fixed" class="footable table table-striped toggle-arrow-tiny table-media-list" style="position: fixed;top: 54px;display: none;margin-left: 15px">
                    <thead>
                        <tr><th data-sort-ignore="true">平台id</th><th data-sort-ignore="true">公众号</th><th data-sort-ignore="true">首选媒体主</th><th data-sort-ignore="true">价格(元)</th><th data-sort-ignore="true">备注</th><th data-sort-ignore="true">审核状态</th><th data-sort-ignore="true">操作时间</th><th data-sort-ignore="true">操作</th></tr>
                    </thead>
                </table>
            </div>
            <?php Pjax::end() ?>
            <table id="header-fixed" class="footable table table-striped toggle-arrow-tiny table-media-list" style="position: fixed;top: 54px;display: none;margin-left: 15px"><thead>
                <tr><th data-sort-ignore="true">平台id</th><th data-sort-ignore="true">公众号</th><th data-sort-ignore="true">首选媒体主</th><th data-sort-ignore="true">价格(元)</th><th data-sort-ignore="true">备注</th><th data-sort-ignore="true">审核状态</th><th data-sort-ignore="true">操作时间</th><th data-sort-ignore="true">操作</th></tr>
                </thead></table>
        </div>
    </div>
        ...
        <div class="modal-content animated fadeIn">
            ...
            <div class="modal-body">
                ...
                <ul class="nav nav-pills">
                    <li class="active">
                        <a class="nav-to-verify-media" href="#stage-to-verify-media-content" data-toggle="tab">审核资源</a>
                    </li>
                    <li>
                        <a class="nav-to-verify-vendor" href="#stage-to-verify-vendor-content" data-toggle="tab">审核媒体主</a>
                    </li>
                    <li>
                                        ...
                                        <div class="form-horizontal">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-md-9 follower-area">
                                                    ...
                                                    <label class="checkbox-inline"><input type="checkbox" class="area-other">其他城市</label>
                                                </div>
                                            </div>
                            ...
                            <div class="panel-body body-add-media-vendor">
                                ...
                                <form class="form-horizontal">
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-1">
                                            <button class="btn btn-primary btn-create-vendor" type="button"">新建媒体主
                                            </button>
                                        </div>
                                    </div>
                                </form>
                                ...
                                <div style="text-align: center">
                                    <button type="button" class="btn btn-success btn-lg btn-add-vendor" style="display: none">添&nbsp;&nbsp;&nbsp;&nbsp;加
                                    </button>
                                </div>
                            </div>
                        </div>
...
<?php
$addMediaVendorJs = <<<JS
    $(".is-pref-vendor").mouseenter(function(){
