$(function(){
    ...
    $('.filter-unlimit').click(function(){
        if($(this).hasClass('filter-active')){
            ...
            switch (_classify_text){
                case '分类：':case '地域：':case '微博认证：':case '粉丝数：':
    ...
    $("a span[data-title]").each(function() {
        ...
        _span.data('data-title', title).hover(function () {
                ...
                $("<div class='show-all-info'>"+title+"</div>").appendTo($(".table-item")).css({ top: offset.top + _span.outerHeight() + 6, left: offset.left + _span.outerWidth() + 1 }).fadeIn(function () {
                });
            },
    ...
    function selectCondition(){
        ...
        switch (_h3_text){
            ...
            case '地域：':
                ...
                break;
            case '微博认证：':
                selectedMore(this,'filter-wb-certification',_h3_text,_this_i);
                break;
            case '粉丝数：':
    ...
    function delCondition(tag){
        ...
        switch (_this_span_text){
            ...
            break;
            case '微博认证：':
                eachDel('filter-wb-certification',_em_text,_min_value,_max_value)
                break;
            case '零售价：':
    ...
    function selectedResource(event){
        ...
        if(_this_checked){
            ...
            var _this_data_value = $(this).parents('tr').find('.account a').attr('data-value');
            $('.resource-table .table tbody tr').each(function(){
                if($(this).find('.account a').attr('data-value') == _this_data_value){
                    $(this).remove();
                }
            });
            var _shopping_tr_length = $('.resource-table tbody tr').length;
