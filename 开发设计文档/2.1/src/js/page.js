...
function page_now_list(number,start,limit)
{
	...
	//执行之间进行清空
	$('.system_page').empty();
	var page_now_number = parseInt(number);
	//开始当前页
	var page_now_start = parseInt(start);
	//每页最多
	...
	if( page_now_number > page_now_limit )
	{
		...
		var str = '<a href="javascript:;" '+number_all+' class="click_prev"> < </a>';
		...
		if( page_now_start<=9)
		{
			if(num_ceil <= 5)
			{
				...
				switch(new_num)
				{
					...
					break;
					default:
                        if(num_ceil >= 15) var db = 15;
                        else var db = num_ceil;
