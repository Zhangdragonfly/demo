$(function(){
    ...
    $('.filter-unlimit').click(function(){
        if($(this).hasClass('filter-active')){
            ...
            switch (_classify_text){
                case '分类：':case '地域：':case '平台：':case '性别：':case '参考报价：':case '粉丝数：':
    ...
    $(document).on('click','.shopping-resource-del',function(){
        ...
        $('.table-item tbody .account').each(function(){
    ...
    function selectCondition(){
        ...
        switch (_h3_text){
            ...
            case '地域：':
                ...
                break;
            case '平台：':
                selectedMore(this,'filter-platform',_h3_text,_this_i);
                break;
            case '性别：':
                selectedMore(this,'filter-sex',_h3_text,_this_i);
                break;
            case '参考报价：':
                selectedOne(this,'参考报价',_h3_text,_this_i);
                break;
            case '粉丝数：':
    ...
    function delCondition(tag){
        ...
        switch (_this_span_text){
            ...
            break;
            case '平台：':
                eachDel('filter-platform',_em_text,_min_value,_max_value)
                break;
            case '性别：':
                eachDel('filter-sex',_em_text,_min_value,_max_value)
                break;
            case '参考报价：':
    ...
    function selectedResource(event){
        ...
        if(_this_checked){
            ...
            var arr = [];
            //把json对象转化成数组，并获取所需要的值
            for(var name in info_obj_new){
                arr.push(name +  ":" + info_obj_new[name]);
            }
            var _fans_num = arr[1].split(':')[1];
            var _platform = arr[2].split(':')[1];
            var _org_price = arr[3].split(':')[1];
            var _offline_price = arr[4].split(':')[1];
            var _sex_num = arr[5].split(':')[1];
            //判断显示平台
            var _platform_name = '';
            var _show_type = '';
            var _price = '';
            ...
            $('.shopping-resource tbody').prepend('<tr>' +
                '<td class="clearfix">'+
                    '<div class="pic-img fl">'+
                         _dt_html+
                        '<i class="car-platform-icon '+_platform_name+'">'+'</i>'+
                    '</div>'+
                    '<ul class="fr ID-info">'+
                        '<li>'+
                            '<a class="synopsis" data-str="6" data-value="'+_data_value+'" data-title="'+_dd_a_attr+'">'+
                                _dd_a_txt+
                            '</a>'+
                            '<i class="sex-icon-g '+_sex_type+'">'+'</i>'+
            ...
            var _this_data_value = $(this).parents('tr').find('dd a').attr('data-value');
            $('.resource-table .table tbody tr').each(function(){
                if($(this).find('.synopsis').attr('data-value') == _this_data_value){
                    $(this).remove();
                }
            });
            var _shopping_tr_length = $('.resource-table tbody tr').length;
